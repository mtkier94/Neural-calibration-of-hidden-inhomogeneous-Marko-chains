...
def create_trainingdata_baseline(frequencies, surv_probs, age_scale):
    ...
    # adjust dtype to list input (to preserve legacy code format)
    cache = list(iter_prod(surv_probs[:,0].tolist(),frequencies))
    y = np.asarray(cache, dtype = 'float32') # adapt to x format
